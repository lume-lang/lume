...
impl Diagnostic {
    ...
    fn span_block(&self) -> syn::Result<TokenStream> {
        let stream = if let Some(span) = self.span() {
            quote! {
                fn span<'a>(&'a self) -> Option<&'a dyn ::diag::source::Source> {
                    let span = (&self.#span) as &'a dyn ::diag::source::Source;
    ...
    fn labels_block(&self) -> syn::Result<TokenStream> {
        ...
        let stream = if let Some(labels) = self.labels() {
            ...
            quote! {
                fn labels<'a>(&'a self) -> Option<Vec<Box<::diag::Label<'a>>>> {
                    let span = &self.#span;
                    let labels = vec![ #(#label_pairs),* ];

                    let labels = labels
                        .into_iter()
                        .map(|(label, range)| Box::new(::diag::Label::new(span, range, label.into())))
                        .collect::<Vec<Box<::diag::Label<'a>>>>();
    ...
    fn severity_block(&self) -> syn::Result<TokenStream> {
        let stream = if let Some(severity) = self.severity() {
            let path: TokenStream = match severity {
                Severity::Error => quote!(::diag::Severity::Error),
                Severity::Warning => quote!(::diag::Severity::Warning),
                Severity::Info => quote!(::diag::Severity::Info),
                Severity::Help => quote!(::diag::Severity::Help),
                Severity::Note => quote!(::diag::Severity::Note),
            };

            quote! {
                fn severity<'a>(&'a self) -> ::diag::Severity {
                    #path
                }
            }
        } else {
