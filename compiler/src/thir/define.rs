...
use crate::hir::{self, ItemId, Symbol};

use super::{ThirBuildCtx, ty::*};
...
impl DefineTypes<'_> {
    fn run(&mut self) -> Result<()> {
        for (_, symbol) in self.symbols {
            match symbol {
                hir::Symbol::Type(t) => self.define_type(&*t)?,
                hir::Symbol::Function(f) => self.define_function(&*f)?,
                _ => (),
            }
        }
    ...
    fn define_type(&mut self, ty: &hir::TypeDefinition) -> Result<()> {
        match &ty {
            hir::TypeDefinition::Class(class) => {
            ...
            hir::TypeDefinition::Alias(_) => return Ok(()),
            _ => todo!(),
        };
    ...
    fn define_function(&mut self, _func: &hir::FunctionDefinition) -> Result<()> {
        Ok(())
    }
}
